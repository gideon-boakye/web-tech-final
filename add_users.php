...
<body>
    ...
    <script>
        $(document).ready(function() {
            $.ajax({
                url: 'database/fetch_roles.php',
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    var dropdown = $('#usertype');
                    dropdown.empty();
                    dropdown.append('<option value="" disabled selected>User Type</option>');
                    $.each(data, function(key, entry) {
                        dropdown.append($('<option></option>').attr('value', entry.typeid).text(entry.rolename));
                    });
                },
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        $.each(data, function(index, user) {
                            ...
                            row += '<td>' + user.updated_at + '</td>';
                            row += '<td>' + user.rolename + '</td>';
                            row += '<td>' +
